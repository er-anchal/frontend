...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-wander-teal shadow-lg py-2' : 'bg-transparent py-4'}`}>
            <div className="container mx-auto px-4 flex justify-between items-center">
                {/* Logo */}
                <a href="/" className="flex items-center">
                    <img src={logo} alt="WanderOn Logo" className="w-13.75 h-auto object-contain" />
                </a>
            ...
            {/* Mobile Menu */}
            {mobileMenuOpen && (
                <div className="lg:hidden bg-wander-teal absolute w-full left-0 top-full p-4 flex flex-col space-y-4 text-white shadow-xl">
                    {['Home', 'Explore', 'About', 'Blog', 'Contact'].map((item) => (
                        <a key={item} href={`/${item.toLowerCase()}`} className="block py-2 border-b border-white/20">
                            {item}
                        </a>
                    ))}
                </div>
